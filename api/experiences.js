...
const dataPath = path.join(process.cwd(), "content", "experience.json");
...
export default async function handler(req, res) {
  if (req.method === "GET") {
    ...
    try {
      ...
      await commitToGithub("content/experience.json", formattedJson);
      
      res.status(200).json({ ok: true, experiences: req.body });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  } else {
